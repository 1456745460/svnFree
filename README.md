...
> SVNFree 是一款 **macOS 原生 SVN 图形客户端**，基于 Python + PyQt6 开发，提供简洁易用的深色主题界面，支持完整的 SVN 工作副本管理功能，并深度集成 macOS Finder 右键菜单。
...
│  菜单栏（仓库 / 工作副本 / 查看 / 帮助）                         │
├─────────────────────────────────────────────────────────────┤
│  工具栏：[更新] [提交] [还原] [日志] [Diff] [添加] [锁定] [刷新] [清理] │
├──────────────────┬──────────────────────────────────────────┤
│  左侧边栏         │  右侧内容区                                │
│  ┌────────────┐  │  ┌──────────────────────────────────────┐ │
│  │  SVNFree  [+]│  │  工作副本文件树                         │ │
│  ├────────────┤  │  ├──────────────────────────────────────┤ │
│  │ 🔍 搜索...  │  │  过滤栏：[仅显示变更] [搜索] [全选] [展开]  │ │
│  ├────────────┤  │  ├──────────────────────────────────────┤ │
│  │ 📁 项目A   │  │  ┌────────────────────┬─────────────────┐│ │
│  │ 📁 项目B   │  │  │ 文件树（名称/状态/  │ 详情面板         ││ │
│  │ 📁 项目C   │  │  │  版本/作者）        │ （状态/路径/     ││ │
...
SVNFree 支持 macOS **Finder 右键菜单（Quick Action）**，可在 Finder 中直接对文件/文件夹执行 SVN 操作，无需打开主窗口。

### 安装 Finder 服务

```bash
cd /path/to/svnFree
python3 install_finder_service.py
```

安装完成后，在 Finder 中右键点击文件或文件夹，在 **`快速操作`** 子菜单中可看到以下选项：

| 菜单项 | 功能 |
|--------|------|
| SVN 提交 | 弹出提交对话框 |
| SVN 更新 | 弹出更新对话框 |
| SVN 查看 Diff | 弹出 Diff 查看器 |
| SVN 查看日志 | 弹出日志对话框 |
| SVN 还原 | 弹出还原确认 |
| SVN 添加 | 将文件加入版本控制 |

> **注意：** 安装后若菜单未出现，请在 **系统偏好设置 → 扩展 → Finder 扩展** 中确认相关服务已启用。
